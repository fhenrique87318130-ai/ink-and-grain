...
import { formatDate, type Article } from "@/lib/articles-shared";
import { getArticleBySlug, listArticles } from "@/lib/articles.functions";
...
export const Route = createFileRoute("/artigo/$slug")({
  loader: async ({ params, context }) => {
    ...
    await context.queryClient.ensureQueryData(articlesQO);
    return { article };
  },
  head: ({ loaderData }) => {
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-screen-md px-4 py-24 text-center">
      <p className="font-mono text-xs uppercase tracking-widest text-accent">Erratum 404</p>
      <h1 className="mt-4 font-serif text-6xl font-black tracking-tighter">Artigo não localizado</h1>
      <p className="mt-4 font-body text-lg text-muted-foreground">A edição que você procura saiu de circulação.</p>
      <Link to="/" className="mt-8 inline-block border border-foreground bg-foreground px-6 py-3 font-sans text-xs font-semibold uppercase tracking-widest text-background hover:bg-background hover:text-foreground">
        Voltar à capa
      </Link>
    </div>
  ),
...
function ArticlePage() {
  ...
  const { data: article } = useSuspenseQuery(articleQO(slug)) as { data: Article };
  ...
  return (
    <article className="mx-auto max-w-screen-xl px-4 py-10">
      <nav className="border-b border-foreground/30 pb-3 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
        ...
        <Link to="/categoria/$category" params={{ category: article.category.toLowerCase() }} className="hover:text-foreground">
          {article.category}
        </Link>
        <span className="mx-2">/</span>
      ...
      <header className="mx-auto mt-10 max-w-4xl text-center">
        <Link to="/categoria/$category" params={{ category: article.category.toLowerCase() }} className="inline-block bg-foreground px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-widest text-background">
          {article.category}
        </Link>
      ...
      {article.hero && (
        <figure className="mx-auto mt-12 max-w-5xl border border-foreground bg-muted">
          <img src={article.hero} alt={article.title} className="aspect-[16/9] w-full object-cover grayscale transition-all duration-700 hover:grayscale-0" />
      ...
      <div className="mx-auto mt-16 max-w-5xl">
        <div className="grid grid-cols-1 gap-x-10 md:grid-cols-3">
          <div className="border-foreground/30 pb-6 md:border-r md:pr-8">
            <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Resumo</p>
            <p className="mt-3 font-serif text-xl font-bold italic leading-snug">"{article.dek}"</p>
          </div>

          <div className="md:col-span-2">
            {article.body.map((p, i) => (
              <p key={i} className={`mb-6 font-body text-lg leading-relaxed text-foreground/90 sm:text-justify ${i === 0 ? "drop-cap" : ""}`}>
                {p}
              </p>
            ))}
            <div className="my-10 text-center font-serif text-xl tracking-[0.5em] text-foreground/40">✦ ✦ ✦</div>
            <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Fim do artigo</p>
          </div>
        </div>
      </div>
      ...
      <nav className="mt-20 grid grid-cols-1 border-y-2 border-foreground sm:grid-cols-2">
        {prev ? (
          <Link to="/artigo/$slug" params={{ slug: prev.slug }} className="group border-foreground p-6 transition-colors hover:bg-foreground hover:text-background sm:border-r sm:p-8">
            <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground group-hover:text-background/60">← Artigo anterior</p>
        ...
        {next ? (
          <Link to="/artigo/$slug" params={{ slug: next.slug }} className="group p-6 text-right transition-colors hover:bg-foreground hover:text-background sm:p-8">
            <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground group-hover:text-background/60">Próximo artigo →</p>
      ...
      {related.length > 0 && (
        <section className="mt-20">
          <div className="mb-8 flex items-end justify-between border-b-2 border-foreground pb-3">
            <h2 className="font-serif text-3xl font-black tracking-tighter sm:text-4xl">Continue lendo em {article.category}</h2>
            <Link to="/categoria/$category" params={{ category: article.category.toLowerCase() }} className="font-mono text-[10px] uppercase tracking-widest underline decoration-2 decoration-accent underline-offset-4">
              Ver tudo
            </Link>
          </div>
